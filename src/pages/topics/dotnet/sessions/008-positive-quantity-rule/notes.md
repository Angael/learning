...
- Follow-up questions show useful TypeScript/ORM transfer: the learner treats database uniqueness as an assumed basic and asks for the EF Core equivalents of a query, immediate insert, and request cancellation. Clarified that `AnyAsync` executes an asynchronous existence query; `Add` only stages an entity in EF Core's unit-of-work/change tracker; and `SaveChangesAsync` persists all staged changes. This is an ORM design choice, not an OOP limitation. Also clarified that request cancellation comes from client disconnect/abort, request timeout, or server shutdown—not from competing requests—and is cooperative: a write may already have committed when cancellation is observed.
