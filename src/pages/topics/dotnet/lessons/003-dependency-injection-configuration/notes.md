...
Correction from follow-up: lesson does include `builder.Services.AddSingleton<AppClock>();`, so `AppClock` is registered as a singleton in this example. The student was right to challenge the earlier correction. Reinforce: singleton means the DI container creates one `AppClock` instance and reuses it; the handler still does not create it.
